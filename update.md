...
## 批次 X+14 · 副窗口 · 00 §9.5 直播 idea 重新插入（2026-06-05）

**背景**

PM 在本地编辑 00 文档时覆盖了批次 X+13 加入的 §9.5「直播间精细化诊断闭环」，文档回到 §9.5 为"没列入下一步的方向"的状态（710 行）。本批次重新插入 §9.5 直播 idea，原 §9.5 边界声明顺移成 §9.6（与 X+13 一致）。

**保留 PM 本次的覆盖改动**：

- §1 标题：「项目是什么（30 秒看懂）」→「项目概括」
- §2 标题：「为什么做这个（产品起点）」→「项目背景」
- 表格列宽对齐符号统一（视觉细节）
...
- §9.5 重新插入「直播间精细化诊断 · 场次 → 动作 → 效果闭环」（93 行，跟 X+13 内容一致）
- 原 §9.5「没列入下一步的方向」顺移成 §9.6

**回归验证**

- ✅ 章节序号 §1→§12 连续，§9.1→§9.6 连续
- ✅ 文档行数 712 → 805
- ✅ 主章节 §10 / §11 / §12 锚点未动

**提交信息**

`docs(overview): restore 9.5 live session loop after local overwrite`

---

## 批次 11 · 开发计划文档 12-从V1到V2（2026-06-10）

**背景**

06-执行计划的 A→B1→B2→B3 主干已全部完成（见 07 测试报告），诊断子图 V1 也已收口（批次 10）。需要一份接棒路线图回答"V1 之后 6-8 周做什么"。基于 codegraph 全量索引核对代码现实（确认 `ReviewQueue/submit` 全仓库零调用、`llm.py` 无 usage 统计、hub 为纯关键词路由等缺口），对照成熟 Agent 工程实践（eval-driven / human-in-the-loop / 可观测 / 成本路由）编写。

**新增**

- `12-开发计划-从V1到V2.md`：
  - §1 现状盘点（8 项已完成 + 8 个缺口 G1-G8 + 3 条结构性边界）
  - M0 商家访谈（前置依赖，最高优先级）
  - M1 质量与治理底座：eval harness / Review 状态机接线 / 运行可观测落库 / make check
  - M2 产品价值升级：三视图话术（00 §9.1）/ 跨子图协同（00 §9.3）/ 成本三杠杆落地（00 §10.3）
  - M3 按 ROI 探索：直播场次闭环 / 双视角选品 / Hub LLM 路由 / MCP 化 / 小二端潜力池
  - §7 分工边界逐条对号 CLAUDE.md；§8 V2 验收口径（V1 五条之上追加 5 条）；§9 风险登记

**回归验证**

- ✅ 纯文档改动，无代码触动
- ✅ 引用的文件（agent/review.py / llm.py / hub.py / native_tools.py / 00/05/06/07/08/10 文档）实际存在

**提交信息**

`docs(plan): add 12 dev plan v1-to-v2 — eval/review/observability + 3 milestones`
